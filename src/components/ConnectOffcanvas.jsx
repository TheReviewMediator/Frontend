...
function ConnectionCards({ platform }) {
  ...
  // API calls
  // These don't work yet. I am waiting for backend support.
  const googleFunction = async () => {
    try {
      const response = await axios.get(
        process.env.BACKEND_URI + `/api/reviews/google/${googleId}`
      );
  ...
  const yelpFunction = async () => {
    ...
    try {
      const response = await axios.get(
        process.env.BACKEND_URI + `/api/reviews/yelp/${yelpId}`
      );
      console.log(response);
    } catch (error) {
      console.error("Error fetching reviews:", error);
  ...
  const facebookFunction = async () => {
    try {
      const response = await axios.get(
        process.env.BACKEND_URI + `/api/reviews/facebook/${facebookId}`
      );
      console.log(response);
    } catch (error) {
      console.error("Error fetching reviews:", error);
  ...
  const trustpilotFunction = async () => {
    try {
      const response = await axios.get(
        process.env.BACKEND_URI + `/api/reviews/trustpilot/${trustpilotId}`
      );
      console.log(response);
    } catch (error) {
      console.error("Error fetching reviews:", error);
  ...
  cards.push({
    name: "Google",
    func: googleFunction,
    updateFunc: updateGoogle,
    isConnected: googleConnected,
  });
  cards.push({
    name: "Yelp",
    func: yelpFunction,
    updateFunc: updateYelp,
    isConnected: yelpConnected,
  });
  cards.push({
    name: "Facebook",
    func: facebookFunction,
    updateFunc: updateFacebook,
    isConnected: facebookConnected,
  });
  cards.push({
    name: "Trustpilot",
    func: trustpilotFunction,
    updateFunc: updateTrustpilot,
    isConnected: trustpilotConnected,
  });
