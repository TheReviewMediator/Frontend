...
const DashboardNavbar = ({ activeLink, sources }) => {
  const { brandImage, setBrandImage, brandName, setBrandName } =
  ...
  return (
    <div>
      <Navbar className={styles.navBar}>
        ...
        >
          <img src={brandImage} height={64} width={64} />
          {brandName}
        </Navbar.Brand>
        ...
        <Nav variant="pills">
          <Nav.Link
            onClick={() => setShowGenReviewModal(true)}
      ...
      <ConnectOffcanvas
        class="offcanvas offcanvas-start w-400 show"
        show={showConnections}
        handleClose={() => setShowConnections(false)}
      />
      <GenerateReviewModal
        sources={sources}
        show={showGenReviewModal}
        handleClose={() => setShowGenReviewModal(false)}
      >
        {" "}
      </GenerateReviewModal>
      <BrandingModal
        show={showBrandingModal}
        handleClose={() => setShowBrandingModal(false)}
      />
    </div>
  );
};
