...
function PolicyForm({ platform, handleClose, alertState, review }) {
  ...
  const handleSubmit = () => {
    ...
    alertState.setter(`Successfully submitted ${selectedReason} policy violation to ${platform}`)
    handleClose()
  }
...
function FlagReviewOffcanvas({ show, handleClose, review, alertState }) {
  return (
    <Offcanvas className={styles.flagOffcanvas} placement={'bottom'} show={show} onHide={handleClose}>
      ...
      <Offcanvas.Body>
        <FlagForm platform={review.platform} handleClose={handleClose} alertState={alertState}></FlagForm>
